...
public class CardLayoutManager extends RecyclerView.LayoutManager {
        ...
        if (itemCount > CardConfig.DEFAULT_SHOW_ITEM) {
            ...
            for (int position = CardConfig.DEFAULT_SHOW_ITEM; position >= 0; position--) {
                ...
                if (position == CardConfig.DEFAULT_SHOW_ITEM) {
                    ...
                    view.setAlpha(1 - (position-1) * CardConfig.DEFAULT_ALPHA);
                    ...
                    view.setAlpha(1 - position * CardConfig.DEFAULT_ALPHA);
    ...
    /*void fillCardLayout(int itemCount, RecyclerView.Recycler recycler, int widthSpace, int heightSpace,int decoratedMeasuredHeight, int decoratedMeasuredWidth) {
...
//                final View view = recycler.getViewForPosition(0);
                ...
                if (position == CardConfig.DEFAULT_SHOW_ITEM) {
                    ...
                    view.setScaleY(1 - (position - 1) * CardConfig.DEFAULT_SCALE);
                    view.setAlpha(1 - (position - 1) * CardConfig.DEFAULT_ALPHA);
                    view.setTranslationY((position - 1) * decoratedMeasuredHeight / CardConfig.DEFAULT_TRANSLATE_Y);
                } else if (position > 0) {
                    view.setScaleX(1 - position * CardConfig.DEFAULT_SCALE);
                    view.setScaleY(1 - position * CardConfig.DEFAULT_SCALE);
                    view.setAlpha(1 - position * CardConfig.DEFAULT_ALPHA);
                    view.setTranslationY(position * decoratedMeasuredHeight/ CardConfig.DEFAULT_TRANSLATE_Y);
                } else {
                    // 设置 mTouchListener 的意义就在于我们想让处于顶层的卡片是可以随意滑动的
                    // 而第二层、第三层等等的卡片是禁止滑动的
                    view.setOnTouchListener(mOnTouchListener);
